...
User profile:
```I am building websites using Wordpress and Elementor. Also I add custom html, css, js and php code to my website```
The user provided the additional info about how they would like you to respond:
```Please always provide full and final instructions```
...
<script>
  ...
  for (const month of months) {
    ...
    createPercentageElement(monthContainer, prevValue, curValue);
    } else if(prevValue && !curValue){
        createBarElement(prevBarContainer, prevValue, 'prev');
        createBarInProgress(curBarContainer, prevValue, 'cur');
        createPercentageInProgress(monthContainer);
    }
  }
...
function createBarElement(parentElement, value, type) {
  ...
  function createPercentageElement(parentElement, prevValue, curValue) {
  ...
  function createPercentageInProgress(parentElement) {
      const percentageElement = document.createElement('div');
...
i need to make some changes into it


Sure, what specific changes would you like to make to this code?


now percentage bubbles are all on the same level. but i want them to be staggered